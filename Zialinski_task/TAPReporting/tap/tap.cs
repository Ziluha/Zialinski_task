...
namespace Taps {
    ...
    public class TAP {
        ...
        static void ReportOkness(string name,bool ok) {
            ...
            {
                Console.Write("not ");
                CreateTapReport.WriteTapResults("not ");
            }
            Console.Write("ok {0}",Cur);
            CreateTapReport.WriteTapResults($"ok {Cur}");
            if (name != null)
            {
                Console.Write(" - {0}",name);
                CreateTapReport.WriteTapResults($" - {name}");
            }
            string intodo=InTodo;
            if(intodo!=null) {
                if(ok) {
                    Console.Write(" # TODO {0} (unexpectedly succeeded)",intodo);
                    CreateTapReport.WriteTapResults($" # TODO {intodo} (unexpectedly succeeded)");
                } else {
                    Console.Write(" # TODO {0}",intodo);
                    CreateTapReport.WriteTapResults($" # TODO {intodo}");
                }
            } else if(Elapsed && time!=TimeSpan.MinValue) {
                if(time.TotalSeconds>=0.001) {
                    Console.Write(" # {0:F6}s",time.TotalSeconds);
                    CreateTapReport.WriteTapResults($" # {time.TotalSeconds:F6}s");
                } else {
                    Console.Write(" # {0:F1}us",time.Ticks/10.0);
                    CreateTapReport.WriteTapResults($" # {time.Ticks / 10.0:F1}us");
                }
            }
            Console.WriteLine();
            CreateTapReport.WriteTapLineResults();
